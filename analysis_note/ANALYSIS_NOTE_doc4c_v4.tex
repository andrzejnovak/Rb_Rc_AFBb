...
%
% Doc 4c v4: FINAL — BDT with SV features + signed thrust axis fix.
...
% Compiled with: tectonic ANALYSIS_NOTE_doc4c_v4.tex
...
$\measured{A_\text{FB}^b = 0.094 \pm 0.005\;\text{(stat)}}$,
...
$> 5$ gives $A_\text{FB}^b = 0.094 \pm 0.005$, within
$0.1\sigma$ of the published ALEPH value.
...
\item \textbf{$A_\text{FB}^b$ breakthrough}: discovered that \texttt{TTheta}
  ...
  Result: $A_\text{FB}^b = 0.094 \pm 0.005$ ($\kappa = 0.3$, WP $> 5$),
  within $0.1\sigma$ of the published ALEPH value ($0.093 \pm 0.005$).
...
\end{equation}
This is within $0.1\sigma$ of the published ALEPH value
$\external{A_\text{FB}^b = 0.0927 \pm 0.0052}$~\cite{ALEPH:AFBb}
(pull $= +0.2\sigma$).

The $\kappa$ dependence of the signed-axis extraction is physical:
at higher $\kappa$, the charge separation $\delta_b$ is larger
but the extracted $A_\text{FB}^b$ decreases because the signing
procedure at high $\kappa$ introduces stronger correlations with
the angular distribution and enhanced sensitivity to non-$b$
contamination.
The optimal extraction uses the lowest $\kappa$ value ($0.3$)
where these effects are minimised.
...
$R_b$ (BDT+SV, tight$=0.80$) & $\measured{0.2155}$ & 0.0004 & --- &
  $\external{0.21578}$ \\
$A_\text{FB}^b$ (signed axis, $\kappa = 0.3$) & $\measured{0.094}$ & 0.005 & --- &
...
\midrule
This analysis (signed, $\kappa=0.3$) & $\measured{0.094}$ & 0.005 &
...
$\measured{0.094 \pm 0.005}$ (at $\kappa = 0.3$, WP $> 5$)
...
\item $R_b = 0.2155 \pm 0.0004\;\text{(stat)}$
  ...
  prediction $R_b^\text{SM} = 0.21578$ (pull $= -0.1\sigma$).
\item $A_\text{FB}^b = 0.094 \pm 0.005\;\text{(stat)}$
...
The result at $\kappa = 0.3$ with a $b$-tag working point
$> 5$ gives $A_\text{FB}^b = 0.094 \pm 0.005$, matching the
published ALEPH result.
