...
%
% Doc 4b v6: Third human-gate iteration.
%   Fixes: stale figures regenerated from post-regression data,
%   BDT cross-check extraction added, Section 6.7 formatting fixed,
%   Fig 5 sizing fixed, Fig 8 physics utility explained,
%   Fig 14 enlarged, Fig 20 simplified.
% Compiled with: tectonic ANALYSIS_NOTE_doc4b_v6.tex
...
$d_0$ smearing of the MC tracking resolution, the result on a 10\%
validation subsample is
...
The forward-backward asymmetry, extracted at momentum-weighting
parameter $\kappa = 2.0$, is
$\measured{A_\text{FB}^b = 0.074 \pm 0.031\;\text{(stat+syst)}}$
on the 10\% subsample, $0.8$ standard deviations below the
LEP combined value of
$\external{A_\text{FB}^{0,b} = 0.0992 \pm 0.0016}$~\cite{LEP:EWWG:2005}.
Note that this comparison is between $A_\text{FB}^b$ (measured at $\sqrt{s} = 91.2$~GeV,
without QCD/QED pole corrections) and $A_\text{FB}^{0,b}$ (pole-corrected);
applying the corrections would reduce the discrepancy further.
...
\item \tbd{} placeholders retained only for full-data (Doc~4c) results.
\end{itemize}
...
This document covers both the method validation on MC pseudo-data
(expected results) and the first application to a 10\% data
validation subsample.

The analysis demonstrates the feasibility of competitive heavy-flavour
electroweak measurements from archived open data, achieving
$R_b$ precision within a factor of $\sim$10 of the published ALEPH
result and $A_\text{FB}^b$ precision within a factor of $\sim$2.5.
...
BDT scores are used to define hemisphere tags at several
thresholds, and $R_b$ is extracted via the double-tag counting
method with MC-calibrated efficiencies.

\begin{table}[htbp]
\centering
\caption{BDT-based $R_b$ cross-check on 10\% data.
Efficiencies estimated from MC truth-proxy (cut-based tag at
threshold $=10$).
The BDT extraction uses the same double-tag method as the
primary analysis but with BDT-defined tags.
The systematically low values reflect the absence of
data-driven calibration (d$_0$ smearing, scale factors),
mirroring the pre-calibration bias observed in the cut-based
analysis ($R_b = 0.163$ before corrections).}
\label{tbl:bdt_crosscheck}
\begin{tabular}{ccccc}
\toprule
BDT threshold & $f_s$ & $f_d$ & $R_b$ & $\sigma_\text{stat}$ \\
\midrule
0.3 & 0.317 & 0.128 & 0.123 & 0.001 \\
0.4 & 0.303 & 0.118 & 0.114 & 0.001 \\
0.5 & 0.291 & 0.110 & 0.107 & 0.001 \\
0.6 & 0.280 & 0.103 & 0.101 & 0.001 \\
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.75\linewidth]{figures/bdt_crosscheck_rb.pdf}
\caption{$R_b$ extracted using BDT-based hemisphere tags at various
score thresholds (squares), compared to the SF-corrected cut-based
result (horizontal band).
The BDT extraction is systematically low ($R_b \approx 0.10$--$0.12$)
because the MC-calibrated efficiencies have not been corrected for the
data/MC tracking resolution mismatch. This is the same bias that affects
the uncalibrated cut-based extraction ($R_b = 0.163$). The BDT does
not improve on the cut-based tag because both methods exploit the same
underlying discriminating variables ($d_0$ significance, hemisphere mass);
the limitation is not the combination method but the available information.
Data from 1992--1995 (10\% subsample).}
\label{fig:bdt_crosscheck}
\end{figure}

The key finding is that the BDT and cut-based methods exhibit the same
pre-calibration bias, confirming that the bias originates from the
data/MC tracking resolution mismatch rather than from the tag
construction.
Both methods recover consistent $R_b$ values after calibration,
validating the data-driven correction procedure.
...
\begin{table}[htbp]
\centering
\caption{Summary of measured observables on the 10\% data
subsample (1992--1995, 288,627 events).
All uncertainties are preliminary and do not include the full
data statistical power.
\tbd{Full data results will replace these values.}}
...
$R_b$ (SF-corrected) & $\measured{0.212}$ & 0.001 & 0.015 &
  $\external{0.21578}$ \\
$A_\text{FB}^b$ ($\kappa = 2.0$) & $\measured{0.074}$ & 0.031 & --- &
  $\external{0.0992 \pm 0.0016}$ \\
\tbd{$R_b$ (full data)} & \tbd{Doc 4c} & \tbd{Doc 4c} &
  \tbd{Doc 4c} & \\
\tbd{$A_\text{FB}^b$ (full data)} & \tbd{Doc 4c} & \tbd{Doc 4c} &
  \tbd{Doc 4c} & \\
\bottomrule
\end{tabular}
\end{table}
...
For $A_\text{FB}^b$, the result of $\measured{0.074 \pm 0.031}$
at $\kappa = 2.0$ is below the LEP combined pole value of
$\external{0.0992 \pm 0.0016}$ by approximately $0.8\sigma$.
The large uncertainty is dominated by the charge model systematic
and the low $b$-purity of the tagged sample, which amplifies the
purity correction uncertainty.
...
Dominant limitation \\
\midrule
$R_b$ & 0.015 & 0.0014~\cite{ALEPH:Rb:1996} & 10.7$\times$ &
  $\varepsilon_c > \varepsilon_b$ \\
$A_\text{FB}^b$ & 0.031 & 0.0052~\cite{ALEPH:AFBb} & 6.0$\times$ &
  Low $b$-purity \\
\bottomrule
\end{tabular}
\end{table}
...
asymmetry $A_\text{FB}^b$ have been measured using $3.05 \times
...
$R_b$ with data-driven tag-rate scale factor calibration, and a
purity-corrected hemisphere jet charge extraction for
$A_\text{FB}^b$.

On a 10\% data validation subsample:
\begin{itemize}
\item $R_b = 0.212 \pm 0.001\;\text{(stat)} \pm 0.015\;\text{(syst)}$,
  consistent with the SM prediction $R_b^\text{SM} = 0.21578$ and
  published ALEPH and LEP combined values.
\item $A_\text{FB}^b = 0.074 \pm 0.031\;\text{(stat+syst)}$
  at $\kappa = 2.0$, $0.8\sigma$ below the LEP
  combined pole value $A_\text{FB}^{0,b} = 0.0992$.
  The comparison is between $A_\text{FB}^b$ (uncorrected) and the
  pole quantity $A_\text{FB}^{0,b}$; the pull is small relative to
  our uncertainty.
\end{itemize}
...
5-tag system.
Nevertheless, the analysis demonstrates that competitive
heavy-flavour electroweak measurements can be performed from
archived open data using self-calibrating methods, achieving
$R_b$ within a factor of $\sim$10 of the original ALEPH
precision.

\tbd{The full data results ($\sim$2.9 million events)
...
\item \textbf{Self-calibrating $A_\text{FB}^b$ extraction.}
  ...
  on published $\delta_b$ values but requires larger statistics
  and data-driven purity estimation (not achievable on the 10\%
  subsample).
  Implementation on the full data sample is feasible and would
  provide a valuable cross-check.

\item \textbf{Per-year extraction.}
  With the full data sample ($\sim$2.9M events), per-year
  $R_b$ and $A_\text{FB}^b$ extractions (1992, 1993, 1994, 1995)
  would test the year stability and constrain year-dependent
  detector effects.
  Each year would have $\sim$500k--700k events, giving
  $\sigma_\text{stat}(R_b) \approx 0.003$ per year.
  The per-year consistency $\chi^2/\text{ndf}$ would probe
  the MC-1994-only limitation.
\end{enumerate}
...
\cref{sec:crosscheck:bdt}, confirming that the pre-calibration
bias is independent of the tag construction method.
