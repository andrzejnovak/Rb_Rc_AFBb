...
%
% Doc 4b v5: Full rewrite from scratch per human gate feedback.
% Compiled with: tectonic ANALYSIS_NOTE_doc4a_v5.tex
...
on the 10\% subsample, approximately $2\sigma$ below the
...
$\Delta R_b = 0.001$.
An alternative angular dependence ($\sin\theta$ vs.\
$\sin^{3/2}\theta$ in \cref{eq:sigma_d0}) gives
$\Delta R_b = 0.0004$.
...
\caption{Hemisphere charge $Q_h$ distribution at $\kappa = 2.0$
in data (1992--1995, 10\%) and MC (1994, normalised to data
integral). The distribution is symmetric about zero, with the
slight asymmetry in data reflecting the physical forward-backward
asymmetry.}
...
robust: the model tension cancels in the $R_b$ extraction.
...
\tbd{Full data results from Phase 4c will replace these values.}}
...
\item $A_\text{FB}^b = 0.074 \pm 0.031\;\text{(stat+syst)}$
  at $\kappa = 2.0$, approximately $2\sigma$ below the LEP
...
sphericity (\cref{fig:datamc_sphericity}),
track $d_0$ (\cref{fig:datamc_d0}),
...
\label{eq:covariance}
\end{equation}
where $\rho \approx 0.15$ is the correlation coefficient arising
...
The full data sample (\tbd{Phase~4c}) will improve the statistical
...
\ref{fig:datamc_sphericity} & Sphericity + $d_0$ & Exploration & 1992--1995 & 1994 \\
...
tectonic analysis_note/ANALYSIS_NOTE_doc4a_v5.tex
\end{verbatim}
