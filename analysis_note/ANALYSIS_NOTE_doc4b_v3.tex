...
%
% Compiled with: cd analysis_note && tectonic ANALYSIS_NOTE_doc4b_v3.tex
...
correction (the full-data analysis). The $\sin^2\theta_\mathrm{eff}^\mathrm{lept}$ derived from
...
\textbf{Impact on the full-data analysis.} The self-calibrating fit must be modified to either
...
the full-data analysis must fix the $\delta_b$ extraction
...
\bottomrule
\multicolumn{5}{l}{\footnotesize $^\dagger$the MC diagnostic: circular MC diagnostic. \quad
$^{*}$Slope detection at $2.4\sigma$; absolute value suppressed by $\delta_b$ bias} \\
\multicolumn{5}{l}{\footnotesize (Section~\ref{sec:delta_b_investigation}). Requires $\delta_b$ correction (the full-data analysis).}
\end{tabular}
\end{table}
...
The the MC diagnostic results are expected values on MC pseudo-data (self-consistency
diagnostics). The the 10\% data validation results on 10\% data provide the first meaningful
...
\textbf{the 10\% data validation (10\% data):} The 10\% data $A_\mathrm{FB}^b = 0.0085 \pm 0.0056$
...
This must be corrected in the full-data analysis via multi-purity fitting or MC-truth calibration.
...
(92\% of $R_b$ variance). The multi-working-point fit in the full-data analysis is critical for
...
The the 10\% data validation 10\% data validation has confirmed the extraction infrastructure works
on real data. The following critical items must be addressed in the full-data analysis (full data):
...
fit. The implementation effort is estimated at one the full-data analysis executor session
