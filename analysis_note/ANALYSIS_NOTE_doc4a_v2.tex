...
%
% Compiled with: cd analysis_note && tectonic ANALYSIS_NOTE_doc4a_v2.tex
...
\begin{figure}[htbp]
\centering
\includegraphics[width=\linewidth]{figures/efficiency_calibration.pdf}
...
$\varepsilon_c$ values (0.43--0.62) confirm that charm hadrons produce significant
lifetime signatures that pass the tag. At WP~10.0 (selected operating point),
$\varepsilon_b = 0.238$, $\varepsilon_c = 0.431$, $\varepsilon_\mathrm{uds} = 0.086$.
No valid calibration solutions exist for WP $< 7$.}
...
Using $\varepsilon_b = 0.193$ and $R_b = 0.280$:
$\partial f_d / \partial C_b = 0.193^2 \times 0.280 = 0.0104$.
The propagation through to $R_b$ gives
$|\mathrm{d}R_b/\mathrm{d}C_b| \approx \delta R_b / \sigma_{C_b} = 0.010 / 0.010 = 1.0$.
...
The impact on $R_b$ is evaluated by re-running the calibration with each form.
...
contribution exceeds $5\times$ the statistical uncertainty ($5 \times 0.031 = 0.155$)
...
\midrule
$\varepsilon_\mathrm{uds}$ & 0.046 (50\%) & 8.48 & 0.387 & $> 5\times$ stat \\
$\varepsilon_c$ & 0.129 (30\%) & 0.60 & 0.078 & --- \\
$C_b$ & 0.010 & 1.03 & 0.010 & --- \\
$R_c$ & 0.003 & 2.53 & 0.008 & --- \\
...
\item WP~9.0: $R_b = 0.347$, pull = $1.93 \pm 0.08$ (pass, $< 2\sigma$),
$n_\mathrm{valid\;toys} = 305/1000$. The pull uncertainty of $\pm 0.08$ is estimated
from the finite toy statistics ($1/\sqrt{305}$); the marginal pass (1.93 vs threshold
2.0) should be interpreted with this uncertainty in mind.
\item WP~10.0: \textbf{INFEASIBLE on the validation split.}
Three attempts were made to obtain a valid closure test at the primary operating point:
\begin{enumerate}
\item \textbf{Standard 60/40 split:} The 40\% validation subset contains
$\sim 292{,}000$ events. At WP~10.0, the toy convergence rate on the full MC is
only $\sim 200/1000$ ($20\%$). On the smaller validation subset, the convergence
rate drops further, producing insufficient valid toys for a meaningful pull
calculation.
\item \textbf{70/30 split (more derivation, less validation):} Increasing the
derivation fraction improves the calibration precision but reduces the validation
statistics further. No valid extraction at WP~10.0.
\item \textbf{Bootstrap closure (overlapping subsets):} Using 80\% derivation and
the full MC as ``validation'' (deliberately circular as a diagnostic) recovers the
full-MC result ($R_b = 0.280$, pull = 0.94 vs input 0.216). This confirms the code
is correct but is not an independent closure test.
\end{enumerate}
The root cause is that WP~10.0 sits at the edge of the physical solution space for
the circular calibration system. The derivation-validation split exacerbates this by
reducing the effective statistics in both halves, pushing more toys into solver failure.
The independent closure at WP~9.0 (which passes) validates the extraction machinery,
but the WP~10.0 closure gap remains an acknowledged limitation.
\end{itemize}
...
the ``Mirrored'' bar has $R_b = 0.0000$ (zero height, annotated with an arrow), while the
full-sample bar (blue, left) shows $R_b \sim 0.83$ from the uncalibrated extraction;
(b) bFlag discrimination power ($\chi^2/\mathrm{ndf} = 11{,}447$,
...
Angular fit $\chi^2$ (intercept) & $\chi^2$/ndf & 26--34/8 & $< 5$ & \textbf{FAIL} ($\chi^2/\mathrm{ndf} \sim 3$--$4$) \\
...
0.280 - 0.216 = 0.064$ arises from the circular calibration procedure. The bias
...
$R_b = 0.216$. The extracted $R_b = 0.280$ is inconsistent with this assumption,
...
\textbf{Risk note:} This fit was downscoped from Phase~4a because the
zero asymmetry on symmetric MC prevents meaningful $\sin^2\theta_\mathrm{eff}$
extraction. Phase~4b must therefore implement AND validate the four-quantity
fit simultaneously, carrying both implementation and validation risk. If
the four-quantity fit fails to converge on data, the simplified linear
regression (validated in Phase~4a) provides a fallback extraction of
$A_\mathrm{FB}^b$ with reduced systematic constraints.
...
\item \textbf{Alternative calibration input ($R_b = 0.280$ instead of SM).}
Using the extracted $R_b = 0.280$ as the calibration input (iterative self-consistency)
was attempted. This converges to $R_b = 0.280$ at WP~10.0 (confirming the circular
...
\textbf{Verdict: INFEASIBLE.} The operating point stability failure is a structural
consequence of the underdetermined 2-equation system with circular calibration. The
multi-working-point fit on data (Phase~4b) is the documented path to resolution, as
data-driven constraints on $\varepsilon_\mathrm{uds}$ enlarge the physical solution
space at multiple WPs.
...
The systematic covariance includes a 10\% $R_b$--$A_\mathrm{FB}^b$ correlation through
