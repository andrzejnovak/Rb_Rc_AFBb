...
%
% Compiled with: cd analysis_note && tectonic ANALYSIS_NOTE_doc4b_v1.tex
...
a $2.4\sigma$ detection of the forward-backward asymmetry, but suppressed $10\times$
relative to the published ALEPH value due to an identified overestimation of the
charge separation $\delta_b = \sigma(Q_h)$;
and $\sin^2\theta_\mathrm{eff}^\mathrm{lept} = 0.248 \pm 0.0007\;\mathrm{(stat)}$
(unreliable due to the $\delta_b$ issue).
...
\item \textbf{Systematic budget updated for 10\% data.} $\varepsilon_\mathrm{uds}$
  now dominates $R_b$ systematics ($\delta R_b = 0.499$, 96\% of budget) because
...
The $\varepsilon_\mathrm{uds}$ systematic now dominates at 96\% of the total variance,
...
$\varepsilon_c$ ($\pm 30\%$) & Background & 0.073 & 0.0\% & Re-extraction \\
...
$\delta R_b = 0.499$ (96\% of budget), followed by $C_b$ ($\delta R_b = 0.124$).
...
($\delta R_b = 0.499$, 96\% of budget), followed by $C_b$ ($\delta R_b = 0.124$, 6\%).
...
$A_\mathrm{FB}^b$ vs ALEPH & Pull & 13.5$\sigma$ & --- & \textbf{KNOWN ISSUE} ($\delta_b$) \\
\bottomrule
\end{tabular}
\end{table}
...
published ALEPH result ($0.0927 \pm 0.0052$), corresponding to a $13.5\sigma$ deviation.
...
value ($0.0927 \pm 0.0052$), with a pull of $13.5\sigma$. This deviation cannot be
...
From the $A_\mathrm{FB}^b$ on 10\% data:
\begin{equation}
\sin^2\theta_\mathrm{eff}^{\mathrm{lept},\,10\%} = 0.248 \pm 0.0007\;\mathrm{(stat)},
\label{eq:sin2theta_10pct}
\end{equation}
from \texttt{parameters.json} (field: \texttt{sin2theta\_eff\_10pct.value} = 0.2484,
\texttt{sin2theta\_eff\_10pct.sigma\_stat} = 0.00065).
The stat-only pull from SM is $|0.248 - 0.232| / 0.0007 = 24\sigma$. This pull is
meaningless because (1) no systematic propagation is included, and (2) the underlying
$A_\mathrm{FB}^b$ is suppressed $10\times$ by the $\delta_b$ overestimation
(Section~\ref{sec:delta_b_investigation}). Phase~4c must fix the $\delta_b$ extraction
...
\begin{tabular}{lcccc}
\toprule
Parameter & Expected (MC) & 10\% data & \tbd{Full data} & SM \\
\midrule
...
$\sin^2\theta_\mathrm{eff}$ & $0.2500 \pm 0.0004$ & $0.248 \pm 0.0007^{*}$ & \tbd{Doc 4c} & 0.23153 \\
$R_c$ & $0.17223 \pm 0.003$ & $0.17223 \pm 0.003$ & \tbd{Doc 4c} & 0.17223 \\
\bottomrule
\multicolumn{5}{l}{\footnotesize $^{*}$Unreliable due to $\delta_b$ overestimation (Section~\ref{sec:delta_b_investigation}).}
\end{tabular}
\end{table}
...
of $13.5\sigma$. This deviation is fully explained by the $\delta_b$ overestimation
...
\midrule
ALEPH~\cite{ALEPH:AFBb} & $0.2330 \pm 0.0009$ & $0.2500 \pm 0.0004$ & $0.248 \pm 0.0007^{*}$ & $^{*}$Unreliable \\
LEP/SLD~\cite{LEP:EWWG:2005} & $0.23153 \pm 0.00016$ & $0.2500 \pm 0.0004$ & $0.248 \pm 0.0007^{*}$ & $^{*}$Unreliable \\
\bottomrule
\end{tabular}
\end{table}
...
(96\% of $R_b$ variance). The multi-working-point fit in Phase~4c is critical for
...
critical for constraining $\varepsilon_\mathrm{uds}$ (currently 96\% of $R_b$
