...
%
% Doc 4b v5: 10% data validation results integrated into the AN.
%   Based on Doc 4a v6 (clean rewrite addressing all arbiter findings).
%   All 10% results sourced from machine-readable JSON:
%     analysis_note/results/parameters.json
%     phase4_inference/4b_partial/outputs/d0_smearing_results.json
%     phase4_inference/4b_partial/outputs/delta_b_calibration.json
%     phase4_inference/4b_partial/outputs/three_tag_results.json
% Compiled with: tectonic ANALYSIS_NOTE_doc4b_v5.tex
...
\caption{Per-flavour hemisphere tagging efficiencies
($\varepsilon_b$, $\varepsilon_c$, $\varepsilon_\text{uds}$) in
the three tag categories (tight, loose, anti-$b$), calibrated on
MC. The anti-$b$ tag is dominated by light-flavour hemispheres
($\varepsilon_\text{uds}^\text{anti} = 0.624$), while the tight
tag has $\varepsilon_b^\text{tight} = 0.497$ but also significant
charm contamination ($\varepsilon_c^\text{tight} = 0.360$).
MC from 1994.}
...
The linear fit (red line) extracts the slope proportional to
$A_\text{FB}^b$.
The negative slope at positive $\cos\theta$ is consistent with
a positive forward-backward asymmetry.}
...
The $B$-hadron lifetime ($\external{\tau_{B^+} = 1.638 \pm 0.004}$~ps,
$\external{\tau_{B^0} = 1.517 \pm 0.004}$~ps,
$\external{\tau_{B_s^0} = 1.516 \pm 0.006}$~ps,
$\external{\tau_{\Lambda_b} = 1.468 \pm 0.009}$~ps~\cite{PDG:2024})
and decay multiplicity ($\external{\langle n_\text{ch}\rangle = 5.36 \pm 0.01}$
per $B$~\cite{PDG:2024}) uncertainties are propagated, giving
$\Delta R_b = 0.0002$.
...
\caption{Systematic uncertainty breakdown for $R_b$ on the 10\%
data subsample. The charm efficiency $\varepsilon_c$ dominates,
reflecting the limited $b$/$c$ discrimination of the
impact-parameter-based tag.
The three-tag system reduced $\varepsilon_c$ from 30\% to 10\%
relative uncertainty, and $\varepsilon_\text{uds}$ from 50\% to
5\% via the anti-tag constraint.}
...
\caption{$A_\text{FB}^b$ as a function of $\kappa$ on the 10\%
data subsample (1992--1995). The SM prediction
...
Higher $\kappa$ values show better consistency due to larger
charge separation $\delta_b$, which reduces the impact of purity
