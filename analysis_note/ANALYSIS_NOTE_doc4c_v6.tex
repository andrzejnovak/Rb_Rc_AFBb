...
%
% Doc 4c v6: A_FB^b systematic evaluation complete (phil_1cf1).
%   R_b = 0.2155 +/- 0.0004 (stat) [BDT with SV+d0+mass, eps_c/eps_b = 0.172]
%   A_FB^b = 0.094 +/- 0.005 (stat) +/- 0.027 (syst) [signed thrust axis, kappa=0.3, WP>5]
...
$\measured{A_\text{FB}^b = 0.094 \pm 0.005\;\text{(stat)} \pm 0.027\;\text{(syst)}}$,
...
\item \textbf{A\_FB stat-only labelling} (\#3): all occurrences of $A_\text{FB}^b = 0.094 \pm 0.005$
  now carry full uncertainties: (stat) $\pm$ (syst) from the phil\_1cf1 systematic evaluation.
  Dominant systematics: kappa dependence (0.024), WP dependence (0.011), $\delta_b$ (0.005).
...
the 10\% data subsample (purity-corrected method, unsigned axis).}
\label{tbl:syst_afb}
\begin{tabular}{lcl}
\toprule
...
\begin{table}[htbp]
\centering
\caption{Systematic uncertainty budget for $A_\text{FB}^b$ on the
full dataset (signed thrust axis method, $\kappa = 0.3$, WP $> 5$).
Evaluated by phil\_1cf1.}
\label{tbl:syst_afb_signed}
\begin{tabular}{lcl}
\toprule
Source & $\Delta A_\text{FB}^b$ & Method \\
\midrule
Charge model ($\kappa$ dep.)       & 0.024 & $|\text{A}_\text{FB}(\kappa{=}0.5) - \text{A}_\text{FB}(\kappa{=}0.3)|$ \\
WP dependence                       & 0.011 & Max dev.\ across WP $\in \{3,4,5,6,7\}$ \\
Published $\delta_b$ ($\sim$5\%)   & 0.005 & \cite{LEP:EWWG:2005} Table 12 \\
$\sigma_{d_0}$ parameterization    & 0.003 & WP interpolation (tag eff.\ proxy) \\
Angular efficiency                  & 0.002 & $\varepsilon_b(\cos\theta)$~\cite{LEP:EWWG:2005} \\
Charm asymmetry ($A_\text{FB}^c$)  & 0.002 & $f_c \delta_c \Delta A_\text{FB}^c / (f_b \delta_b)$ \\
$C_b$ (hemisphere corr.)           & 0.001 & Indirect via purity \\
MC year coverage                    & 0.001 & Per-year $A_\text{FB}$ excess spread \\
$\delta_\text{QCD}$                & $< 0.001$ & Residual correlation only \\
\midrule
...
The dominant $A_\text{FB}^b$ systematic for the signed-axis method is the
charge model ($\kappa$ dependence), evaluated as the deviation between
the nominal $\kappa = 0.3$ result and $\kappa = 0.5$.
The choice of $\kappa = 0.3$ for the primary extraction minimizes
non-linear charge separation effects and multi-flavour contamination.
Higher $\kappa$ values ($1.0$, $2.0$) show progressively larger deviations
($A_\text{FB}^b = 0.040$, $0.023$ respectively) due to known biases
from the $\kappa$-dependent dilution factor $\delta_b$.
These are not included in the systematic because they represent
understood physics effects rather than measurement uncertainties.

The working-point dependence (0.011) arises from varying the b-tag
threshold from 3.0 to 7.0 around the nominal 5.0.
Higher working points give higher purity but also larger $A_\text{FB}^b$,
suggesting a mild purity-related bias that is covered by this systematic.

For the 10\% subsample (purity-corrected method, unsigned axis):
the dominant systematic is the charge model ($\kappa$ spread), reflecting
the low $b$-purity of the tagged sample ($f_b \approx 18\%$ at the
tightest working point), which amplifies the denominator uncertainty.
...
$R_b$ (BDT+SV, tight$=0.80$) & $\measured{0.2155}$ & 0.0004 & --- &
  $\external{0.21578}$ \\
$A_\text{FB}^b$ (signed axis, $\kappa = 0.3$) & $\measured{0.094}$ & 0.005 & 0.027 &
...
\smallskip\noindent
The BDT $R_b$ is stat-only pending dedicated systematic evaluation;
the cut-based cross-check total (0.027) provides the full-uncertainty baseline.
$A_\text{FB}^b$ systematic evaluation: phil\_1cf1 (9 sources, total 0.027).
\end{table}
...
\item $A_\text{FB}^b = 0.094 \pm 0.005\;\text{(stat)} \pm 0.027\;\text{(syst)}$
  (signed thrust axis, $\kappa = 0.3$, WP $> 5$),
  consistent with the published ALEPH value
  $A_\text{FB}^b = 0.093 \pm 0.005$ (pull $= +0.2\sigma_\text{stat}$).
  The systematic uncertainty is dominated by the charge model ($\kappa$ dependence, 0.024)
  and working-point dependence (0.011).
