...
%
% Doc 4c v3: Post-arbiter fixes (5A + 9B + 8C from AN_ARBITER_ada_0141).
%   R_b = 0.21236 +/- 0.00010 (stat) +/- 0.027 (syst) [SF-calibrated, C_b=1.0, combined 15 configs]
%   R_b stability: chi2/ndf = 4.4/14 (p=0.99) -- PASS.
%   A_FB^b (kappa=2.0) = +0.014 +/- 0.005 (PRIMARY).
%   A_FB^b (combined) = +0.0025 +/- 0.0026 (stat) +/- 0.0021 (syst) [cross-check].
%   Per-year consistency: chi2/ndf = 3.57/3 (R_b), 3.82/3 (AFB) -- both PASS.
%   Closure test on full data: max|pull| = 2.80, PASS.
%   Fixes: ALEPH labels on 4 figures, per-year table note, systematics.json,
...
%     COMMITMENTS.md, C1-C8 minor fixes.
% Compiled with: tectonic ANALYSIS_NOTE_doc4c_v3.tex
...
$\measured{R_b = 0.21236 \pm 0.00010\;\text{(stat)} \pm 0.027\;\text{(syst)}}$
(combined across fifteen working-point configurations with
$C_b = 1.0$), stable with $\chi^2/\text{ndf} = 4.4/14$
($p = 0.99$).
The result is consistent with the Standard Model prediction of
$\external{R_b^\text{SM} = 0.21578}$~\cite{LEP:EWWG:2005},
validating the SF calibration across the full range of
operating points.
The forward-backward asymmetry, extracted using the purity-corrected
method (slope$/f_b \delta_b$, no charm subtraction) with published
charge separations, is
$\measured{A_\text{FB}^b(\kappa = 2.0) = +0.014 \pm 0.005\;\text{(stat)}}$,
where $\kappa = 2.0$ provides the best $b$/$c$ charge separation
and is adopted as the primary result.
A cross-check combining all four $\kappa$ values gives
$A_\text{FB}^b = +0.0025 \pm 0.0026\;\text{(stat)}
\pm 0.0021\;\text{(syst)}$,
but the cross-$\kappa$ combination has $\chi^2/\text{ndf} = 10.9/3$
($p = 0.012$), reflecting genuine $\kappa$-dependent charm
contamination that makes the combined value less informative
than the single-$\kappa$ result.
The positive sign is correct; the magnitude reflects dilution
by charm contamination and the absence of a charm subtraction,
which would require reliable per-working-point purity estimates.
The expected diluted asymmetry is
$A_\text{FB}^{b,\text{diluted}} \approx A_\text{FB}^{b,\text{true}}
\times f_b \times \delta_b \approx 0.103 \times 0.18 \times 0.13
\approx 0.002$, consistent with the observed $+0.0025$.
$R_c$ is constrained to the SM value; the data do not independently
constrain it (floating $R_c$ gives
$\sigma(R_c) > 0.05$, reflecting the limited $b$/$c$ separation).
...
using the SF-calibrated three-tag system (with $C_b = 1.0$)
for $R_b$ and the purity-corrected method
(slope$/f_b \delta_b$, no charm subtraction) for $A_\text{FB}^b$.
...
events) using the SF-calibrated three-tag system with $C_b = 1.0$.
Representative subset of fifteen (tight, loose) threshold
configurations; all yield $R_b \approx 0.212$.}
...
\midrule
\textbf{tight$=8$, loose$=4$}  & $\measured{\mathbf{0.2123}}$ &
  \textbf{0.0004} & 0.1967 & 443/7 &
  0.973 / 0.995 / 1.017 \\
tight$=9$, loose$=4$  & $\measured{0.2123}$ & 0.0004 & 0.1957 &
  381/7 & 0.969 / 0.995 / 1.017 \\
tight$=9$, loose$=5$  & $\measured{0.2125}$ & 0.0004 & 0.1956 &
  383/7 & 0.969 / 0.994 / 1.014 \\
tight$=10$, loose$=5$ & $\measured{0.2124}$ & 0.0004 & 0.1945 &
  354/7 & 0.963 / 0.995 / 1.014 \\
tight$=10$, loose$=3$ & $\measured{0.2121}$ & 0.0004 & 0.1930 &
  293/7 & 0.963 / 0.997 / 1.021 \\
tight$=12$, loose$=6$ & $\measured{0.2125}$ & 0.0004 & 0.1920 &
  296/7 & 0.949 / 0.996 / 1.012 \\
tight$=14$, loose$=5$ & $\measured{0.2123}$ & 0.0004 & 0.1912 &
  361/7 & 0.937 / 0.994 / 1.014 \\
\bottomrule
\end{tabular}
\end{table}

The combined result across all fifteen configurations (with $C_b = 1.0$) is:
\begin{equation}
\boxed{
\measured{R_b^\text{combined} = 0.21236 \pm 0.00010\;\text{(stat)}}
}
\label{eq:rb_combined_fulldata}
\end{equation}
with a stability $\chi^2/\text{ndf} = 4.4/14$ ($p = 0.99$) --- PASS.
The residual per-WP variations are dominated by the
single-tag/double-tag tension on $\varepsilon_\text{uds}$,
which shifts by $\sim$3\% across threshold configurations.
The excellent stability demonstrates that the SF calibration
effectively corrects data/MC efficiency differences across the
full range of working points.

At the best single configuration (tight $= 8$, loose $= 4$):
\begin{equation}
\measured{R_b(\text{tight}=8,\text{loose}=4) =
0.21226 \pm 0.00036\;\text{(stat)}}
\label{eq:rb_best_fulldata}
\end{equation}
...
\begin{equation}
\boxed{
\measured{R_b = 0.21236 \pm 0.00010\;\text{(stat)} \pm
0.027\;\text{(syst)}}
}
...
\caption{SF-calibrated $R_b$ (with $C_b = 1.0$) as a function of
working-point configuration on the full ALEPH 1992--1995 dataset
(2,887,261 events).
The dashed line shows the SM prediction
$R_b^\text{SM} = 0.21578$.
All fifteen configurations are consistent with the SM value:
$\chi^2/\text{ndf} = 4.4/14$ ($p = 0.99$).
The combined result is $R_b = 0.21236 \pm 0.00010$.}
...
raw MC efficiencies ($R_b \approx 0.188$--$0.197$),
SF-calibrated with $C_b = 1.0$ ($R_b \approx 0.212$, combined
$0.21236$).
...
\textbf{Method.}
The primary $A_\text{FB}^b$ extraction uses the \emph{purity-corrected}
method (slope$/f_b \delta_b$), matching the approach validated on the
10\% subsample and approved at the human gate.
No charm subtraction is applied, because the purity estimation
function only has reliable calibration at two working points (9.0 and
10.0); applying the full charm correction
$f_c \delta_c A_\text{FB}^c$ at all WPs produces unstable results.
The inclusive method (slope$/\delta_b$) serves as a cross-check.

\begin{table}[htbp]
\centering
\caption{Purity-corrected $A_\text{FB}^b$ (slope$/f_b \delta_b$,
no charm subtraction) on full data (1992--1995, 2,887,261 events),
combined across working points for each $\kappa$.
...
\midrule
0.3 & 0.162 & $\measured{-0.0087}$ & 0.0063 & $4.1/5$ \\
0.5 & 0.233 & $\measured{-0.0047}$ & 0.0052 & $4.2/5$ \\
1.0 & 0.374 & $\measured{+0.0047}$ & 0.0050 & $2.3/5$ \\
2.0 & 0.579 & $\measured{+0.0142}$ & 0.0050 & $0.5/5$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{equation}
\boxed{
\measured{A_\text{FB}^b = +0.0025 \pm 0.0026\;\text{(stat)}
\pm 0.0021\;\text{(syst)}}
}
\label{eq:afb_combined_fulldata}
\end{equation}
with $\chi^2/\text{ndf} = 10.9/3$ ($p = 0.012$), reflecting
the genuine $\kappa$ dependence.
At $\kappa = 2.0$, where the $b$/$c$ charge separation is
largest:
\begin{equation}
\boxed{
\measured{A_\text{FB}^b(\kappa = 2.0) = +0.014 \pm 0.005
\;\text{(stat)}}
}
\label{eq:afb_k2_fulldata}
\end{equation}

The $\kappa$ dependence (from $-0.009$ at $\kappa = 0.3$ to
$+0.014$ at $\kappa = 2.0$) confirms the expected charm dilution
...
The combined value ($+0.0025$) is smaller than the LEP
combined $A_\text{FB}^{0,b} = 0.0992$ because the purity-corrected
method without charm subtraction does not fully isolate the
$b$-quark asymmetry.
At $\sim$20\% $b$-purity, charm contamination dilutes the signal.
Quantitatively, the expected diluted asymmetry is
$A_\text{FB}^{b,\text{diluted}} \approx A_\text{FB}^{b,\text{true}}
\times f_b \times \delta_b \approx 0.103 \times 0.18 \times 0.13
\approx 0.002$, consistent with the observed $+0.0025$.
The inclusive cross-check gives
$A_\text{FB}^b = +0.0005 \pm 0.0005$, further diluted because
it divides only by $\delta_b$ without the purity correction.
...
extracted using the purity-corrected method
(slope$/f_b \delta_b$, no charm subtraction).
...
A_\text{FB}^{0,b} &= \frac{A_\text{FB}^b}{1 - \delta_\text{QCD}
  - \delta_\text{QED}} = \measured{+0.0026 \pm 0.0027}\,,\\
\sin^2\theta_\text{eff}^\text{lept} &= \measured{0.2495 \pm
  0.0005\;\text{(stat)}}\,,
\end{align}
where $\external{\delta_\text{QCD} = 0.0356 \pm 0.0029}$ and
...
$A_\text{FB}^b$ (without charm subtraction) propagates into an
...
(purity-corrected, $\kappa = 2.0$) on full ALEPH data.
...
\midrule
\multicolumn{5}{l}{\textit{Full data (FINAL --- CORRECTED)}} \\
$R_b$ (SF, combined, $C_b=1.0$) & $\measured{0.21236}$ & 0.00010 & 0.027 &
  $\external{0.21578}$ \\
$R_b$ (SF, tight$=8$, loose$=4$) & $\measured{0.21226}$ & 0.00036 & --- &
  $\external{0.21578}$ \\
$A_\text{FB}^b$ (purity-corr., combined) & $\measured{+0.0025}$ & 0.0026 & 0.0021 &
  $\external{0.0992 \pm 0.0016}$ \\
$A_\text{FB}^b$ (purity-corr., $\kappa = 2.0$) & $\measured{+0.014}$ & 0.005 & --- &
  $\external{0.0992 \pm 0.0016}$ \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
This analysis (full data, combined) & $\measured{0.21236}$ & 0.027 &
  3-tag, SF-calibrated, $C_b=1.0$ \\
This analysis (full data, tight$=8$) & $\measured{0.21226}$ & 0.00036 &
  3-tag, SF-calibrated \\
...
\midrule
This analysis (full, combined) & $\measured{+0.0025}$ & 0.0034 &
  Purity-corrected (no charm) \\
This analysis (full, $\kappa=2.0$) & $\measured{+0.014}$ & 0.005 &
  Purity-corrected (no charm) \\
...
The $R_b$ result of $\measured{0.21236 \pm 0.027}$ is consistent
...
The total uncertainty is a factor of $\sim$19 larger than the
...
For $A_\text{FB}^b$, the full-data purity-corrected result of
$\measured{+0.0025 \pm 0.0034}$ (combined) is below the LEP
combined pole value of
$\external{A_\text{FB}^{0,b} = 0.0992 \pm 0.0016}$.
This is expected: the purity-corrected method without charm
subtraction does not fully isolate the $b$-quark asymmetry.
At $\kappa = 2.0$, where the $b$/$c$ charge separation is
largest, $A_\text{FB}^b = +0.014 \pm 0.005$ (stat), showing
the correct positive sign and a value closer to the published
result.
The result on the 10\% subsample ($0.074 \pm 0.031$) was
consistent with the full-data extraction within the large
statistical uncertainty of the subsample.
...
Dominant limitation \\
\midrule
$R_b$ & 0.027 & 0.0014~\cite{ALEPH:Rb:1996} & 19$\times$ &
  $\varepsilon_c \approx \varepsilon_b$ \\
$A_\text{FB}^b$ & 0.0034 & 0.0052~\cite{ALEPH:AFBb} & 0.7$\times$ &
  Large stat.\ on diluted asymmetry \\
\bottomrule
\end{tabular}
\end{table}
...
$R_b$ with data-driven tag-rate scale factor calibration
($C_b = 1.0$), and a purity-corrected hemisphere jet charge
extraction (slope$/f_b \delta_b$, without charm subtraction) for
$A_\text{FB}^b$.

The final results on the complete dataset are:
\begin{itemize}
\item $R_b = 0.21236 \pm 0.00010\;\text{(stat)} \pm 0.027\;\text{(syst)}$
  (combined across fifteen working-point configurations),
  stable with $\chi^2/\text{ndf} = 4.4/14$ ($p = 0.99$).
  The result is consistent with the SM prediction
  $R_b^\text{SM} = 0.21578$ within $1.3\sigma$.
\item $A_\text{FB}^b(\kappa = 2.0) = +0.014 \pm 0.005\;\text{(stat)}$
  (primary result, where $b$/$c$ charge separation is largest).
  The cross-$\kappa$ combination gives
  $A_\text{FB}^b = +0.0025 \pm 0.0026\;\text{(stat)} \pm
  0.0021\;\text{(syst)}$, but with $\chi^2/\text{ndf} = 10.9/3$
  ($p = 0.012$) the combination is presented as a cross-check
  only, reflecting $\kappa$-dependent charm contamination.
  The positive sign is correct; the magnitude reflects dilution
  from charm contamination in the absence of a charm subtraction.
  The expected diluted asymmetry ($A_\text{FB}^{b,\text{true}}
  \times f_b \times \delta_b \approx 0.103 \times 0.18 \times 0.13
  \approx 0.002$) is consistent with the observed $+0.0025$.
...
5-tag system.
The charm efficiency systematic ($\Delta R_b = 0.017$) dominates
the total uncertainty and is the precision bottleneck.

The $A_\text{FB}^b$ result is limited by the inability to perform
reliable charm subtraction at arbitrary tag thresholds:
the purity estimation function has calibration at only two working
points, making the full charm correction unstable.
The purity-corrected method without charm subtraction yields a
diluted asymmetry ($+0.0025$ combined, $+0.014$ at $\kappa = 2.0$)
but with the correct sign and consistent $\kappa$ dependence.
...
The stable recovery of $R_b \approx 0.212$ across all fifteen
working points validates the SF calibration approach and suggests
that improved $b$/$c$ discrimination (e.g., via secondary vertex
