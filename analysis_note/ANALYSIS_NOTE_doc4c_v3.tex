...
%
% Doc 4c v2: SF-calibrated corrected full-data results.
%   R_b = 0.21236 +/- 0.00010 (stat) +/- 0.027 (syst) [SF-calibrated, C_b=1.0, combined 15 configs]
%   R_b stability: chi2/ndf = 4.4/14 (p=0.99) -- PASS.
%   A_FB^b = +0.0025 +/- 0.0026 (stat) +/- 0.0021 (syst) [purity-corrected, no charm].
%   A_FB^b (kappa=2.0) = +0.014 +/- 0.005.
%   Per-year consistency: chi2/ndf = 3.57/3 (R_b), 3.82/3 (AFB) -- both PASS.
%   Closure test on full data: max|pull| = 2.80, PASS.
%   CORRECTED from v1: restored SF-calibrated R_b with 15-config stability,
%     restored purity-corrected AFB method, fixed 6 figures.
% Compiled with: tectonic ANALYSIS_NOTE_doc4c_v2.tex
...
or $\measured{A_\text{FB}^b = +0.014 \pm 0.005\;\text{(stat)}}$
at $\kappa = 2.0$.
...
loose$=4$; $A_\text{FB}^b$ uses the purity-corrected method at
$\kappa = 2.0$.}
...
\item $A_\text{FB}^b = +0.0025 \pm 0.0026\;\text{(stat)} \pm
  0.0021\;\text{(syst)}$ (combined across four $\kappa$ values).
  At $\kappa = 2.0$:
  $A_\text{FB}^b = +0.014 \pm 0.005\;\text{(stat)}$.
  The positive sign is correct; the magnitude reflects dilution
