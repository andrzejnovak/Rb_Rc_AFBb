...
% Compile with: tectonic ANALYSIS_NOTE_doc4c_v9.tex
...
$|\cos\theta|$ extraction, this symmetric component is absorbed into
the fit intercept and does not bias the slope.

\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/afb_angular_quadratic.pdf}
\caption{Comparison of linear and quadratic fit models for the
...
The asymmetry is extracted from a weighted linear fit of
...
\item Events are binned in 10 equal-width bins of $|\cos\theta|$.  In
  ...
  asymmetry.
\item A weighted linear fit $\langle Q_\mathrm{FB} \rangle = a + b \cdot
  |\cos\theta|$ is performed.  The slope $b$ equals
  $\delta_b \cdot A_\mathrm{FB}^b$.
\end{enumerate}

At $\kappa = 0.3$ and WP~$> 5$, the fit gives
$\chi^2/\mathrm{ndf} = \measured{7.1/9}$ ($p = 0.63$), indicating good
linearity.  The slope is divided by the published
...
that the linear model is adequate for the $|\cos\theta|$ extraction,
with an F-test $p$-value of 0.084 for the additional $\cos^2\theta$ term.
The quadratic model gives a consistent (but less precise) result.
