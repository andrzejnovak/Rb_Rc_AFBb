...
% Session: hugo_1b90 (arbiter-fixer) | Date: 2026-04-04
% Based on v7 by florence_dc53, all review findings addressed.
% All numbers from analysis_note/results/*.json — never from memory.
% Compile with: tectonic ANALYSIS_NOTE_doc4c_v8.tex
...
\begin{figure}[htbp]
\centering
\includegraphics[height=0.45\linewidth]{figures/afb_angular_fulldata.pdf}
\caption{The mean charge asymmetry $\langle Q_\mathrm{FB} \rangle$ as a
  function of $\cos\theta$ at $\kappa = 0.3$ and working point
  threshold~$> 5$, with the linear fit overlaid.  The slope is
  consistent with zero within uncertainties, reflecting the small
  magnitude of the charge-flow signal on the full dataset.  The fit
  yields $\chi^2/\mathrm{ndf} = 66.8/8$; the poor goodness-of-fit
  indicates residual structure not captured by the linear model.
...
smaller $\varepsilon_c$ systematic if evaluated independently, but the
BDT-specific systematic evaluation has not been performed.  The cut-based
systematic budget is transferred as an upper bound.  The BDT training
sample is defined by the combined tag threshold (combined tag $> 10$ for
$b$-enriched, $< 2$ for light-enriched).  When $\varepsilon_c$ is varied
by $\pm 10\%$, the training sample composition changes slightly, but the
same trained BDT is applied (the BDT is not retrained per systematic
variation).  The closure test on an independent MC split confirms unbiased
extraction under this scheme (Table~\ref{tbl:closure}).
...
\textbf{Note on the per-year $R_b$ values.}
The per-year results ($R_b \sim 0.188$) use the cut-based combined tag
...
$R_b = 0.2159$, consistent with the BDT result.  The per-year
cross-check uses the looser WP because it provides sufficient events
per year for a meaningful year-by-year comparison, and the purpose is to
test year-to-year \emph{consistency}, not the absolute $R_b$ value.
The BDT-based per-year extraction is not shown because the BDT was
trained on the combined multi-year sample and per-year evaluation would
require retraining.
...
cd analysis_note
tectonic ANALYSIS_NOTE_doc4c_v8.tex
\end{verbatim}
