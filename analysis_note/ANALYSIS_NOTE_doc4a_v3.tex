...
$C_b$ from correlation measurement. At WP~10.0: $\varepsilon_b = 0.151$.
...
point. At WP~10.0: $\varepsilon_\mathrm{uds} = 0.114$.}
...
quadratic discriminant becomes negative). The dominant systematic is now $\varepsilon_c$ (0.201).
...
contributions. The $\varepsilon_\mathrm{uds}$ systematic dominates at $\delta R_b = 0.000$ (solver failure at varied values),
...
the $R_b$ uncertainty at 93\% of the total systematic variance, rendering the measurement insensitive
...
$C_b$ (0.049), $R_c$ (0.020), and $\varepsilon_c$ (0.201 at $-30\%$). The strategy
for constraining $\varepsilon_c$ from data (Phase~4b/4c) is the multi-working-point
simultaneous fit, which constrains $\varepsilon_c$ and $\varepsilon_\mathrm{uds}$ by
requiring consistency across working points. \textbf{Caveat:} the $\sim 10\times$ improvement
claimed for the multi-WP fit assumes smooth efficiency dependence on the working point threshold.
This smoothness assumption has not been validated at Phase~4a; it will be tested on data at
Phase~4b. For $A_\mathrm{FB}^b$, the dominant systematic is the charm
...
\item WP~9.0: $R_b = 0.371$, pull = $1.03$ (PASS, $< 2\sigma$).
...
\midrule
Indep. closure (WP~9.0) & pull & 1.03 & $< 2$ & PASS \\
Indep. closure (WP~10.0) & pull & 1.06 & $< 2$ & PASS \\
Corrupted corrections & N sensitive & 5/6 & $\geq 4/6$ & PASS \\
Per-year consistency & $\chi^2$/ndf & 0.76/3 & $p > 0.05$ & PASS ($p = 0.86$) \\
...
\midrule
$R_b$ & $0.310 \pm 0.029 \pm 0.208$ & \tbd{Doc 4b} & \tbd{Doc 4c} & 0.21578 \\
...
The expected $R_b = 0.310 \pm 0.210$ is consistent with all reference values within
$1\sigma$ due to the very large total uncertainty:
\begin{itemize}
\item vs ALEPH ($0.2158 \pm 0.0014$): pull = $(0.310 - 0.216)/0.210 = 0.45$
...
\item $R_b$: our total / LEP combined total = $0.210/0.00066 = 318\times$.
...
total uncertainty ($\sigma_\mathrm{total} = 0.210$). This table will be updated with
...
\midrule
ALEPH~\cite{ALEPH:Rb:1996} & $0.2158 \pm 0.0014$ & $0.310 \pm 0.210$ & 0.45 & Uninformative \\
ALEPH precise~\cite{ALEPH:Rb:precise} & $0.2159 \pm 0.0014$ & $0.310 \pm 0.210$ & 0.45 & Uninformative \\
LEP combined~\cite{LEP:EWWG:2005} & $0.21629 \pm 0.00066$ & $0.310 \pm 0.210$ & 0.45 & Uninformative \\
DELPHI~\cite{DELPHI:Rb} & $0.21625 \pm 0.00091$ & $0.310 \pm 0.210$ & 0.45 & Uninformative \\
SM prediction~\cite{LEP:EWWG:2005} & 0.21578 & $0.310 \pm 0.210$ & 0.45 & Uninformative \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{enumerate}
\item The double-tag method correctly extracts $R_b = 0.310 \pm 0.029\;\mathrm{(stat)}
\pm 0.208\;\mathrm{(syst)}$ as a self-consistency diagnostic. The large systematic
is dominated by $\varepsilon_c$ (93\% of budget), which will be constrained
...
\item Independent closure passes at both WP~9.0 (pull = 1.03) and WP~10.0 (pull = 1.06)
after the $C_b$ working-point correction, and corrupted-correction
sensitivity tests pass (5/6 sensitive).
\end{enumerate}
...
\textbf{Phase~4b blocking gates:} The following items must be verified on 10\% data
before proceeding to full data (Phase~4c):
\begin{itemize}
\item Operating point stability: at least 2 of 4 WPs must give valid extractions
with consistent $R_b$ ($\chi^2$ p-value $> 0.05$). If 1/4 persists on data,
the multi-WP fit strategy must be reconsidered.
\item $\varepsilon_c$ constraint from data: the dominant systematic (0.201) must be
reduced by at least $3\times$ through the data-driven multi-WP constraint.
\item Intercept $\chi^2/\mathrm{ndf}$ for $A_\mathrm{FB}^b$: if the GoF failure
persists on data, a formal investigation of the cos$\theta$-dependent bias must
be completed before the result is quoted.
\end{itemize}
...
dominant $\varepsilon_c$ systematic from $\sim 0.201$ to $\sim 0.02$, a $\sim 10\times$ improvement.
...
$R_b = 0.310$, $A_\mathrm{FB}^b = -0.0001$.
